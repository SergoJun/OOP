...
public:
	...
	void operator=(Conteiner<T>& A) { this->tmp = this->head; if (this->count == nullptr || this->count->i == 1) {while (this->tmp != nullptr) { this->head = this->head->next; /*std::cout << " удалено ";*/ delete this->tmp; this->tmp = this->head; }
		if (this->count != nullptr) { delete this-> count; this->count =nullptr; }}if (this->count != nullptr) { this->count->i--; this->head = nullptr; }A.tmp = A.head; this->head = nullptr; this->tail = nullptr;
	...
	void delite_elem(int i) {
		int j = 0; tmp = head; while ((j != i) && (tmp != nullptr)) { ++j; tmp = tmp->next; } if (tmp != nullptr) {if ((tmp == head) && (tmp == tail)) { head = nullptr; tail = nullptr; delete tmp; }else if (tmp->next == nullptr) { tmp_buf = tmp->prev; tmp_buf->next = nullptr; delete tmp; tail = tmp_buf; }
		else if (tmp->prev == nullptr) { tmp_buf = tmp->next; tmp_buf->prev = nullptr; delete tmp; head = tmp_buf ;}else { tmp_buf = tmp->next; tmp->prev->next = tmp_buf; tmp_buf->prev = tmp->prev; delete tmp; }}};
